...
ls -l /nonexistent 2>&1 | tee /dev/tty | grep "No such file"
В этом примере я использую команду ls для вывода списка файлов и каталогов в директории /nonexistent, которой не существует. Это приведет к ошибке, которая будет выведена в stderr.

Затем я использую оператор перенаправления stderr (2) в stdout (1) и передаю его на вход tee. Флаг -a утилиты tee указывает ей дополнительно сохранить данные в файле. В данном случае используется специальный файл /dev/tty, который ссылается на терминал, и вывод из stdout будет передан на терминал.

Далее я использую команду grep для поиска строки "No such file" только в stderr и передачи ее на следующую команду в конвейере.

Таким образом, на терминале будут отображены и stdout и stderr команды, но на конвейер будет передана только stderr, содержащая сообщение об ошибке "No such file".
