...
Команда cd является командой типа OUT, так как она изменяет текущий каталог (working directory) для текущего сеанса командной оболочки. Она не производит вывода на экран или какого-либо другого внешнего эффекта, кроме изменения текущего каталога.
...
Да, это возможно с помощью оператора tee, который записывает в стандартный вывод и в файл одновременно. Вот пример команды, которая читает содержимое файла input.txt из стандартного ввода и записывает его в файл output.txt и в стандартный вывод: cat input.txt | tee output.txt

Если мы хотим, чтобы вывод команды ls записывался в файл output.txt, а содержимое файла input.txt передавалось на stdin команды, то можно воспользоваться командой cat, чтобы объединить файл и вывод ls: cat input.txt | ls | tee output.txt

В результате в файл output.txt будет записан вывод команды ls, а также содержимое файла input.txt, переданное на stdin команды ls.
...
Да, можно использовать только stderr в качестве входного потока для pipe, не потеряв отображение stdout на pty. Для этого нужно перенаправить stdout на stderr в команде, используя дескриптор файлов. Например, если у нас есть команда command, выводящая результат на stdout и ошибки на stderr, и мы хотим передать только stderr в pipe, то можно использовать следующую команду: command 2>&1 1>&2 | some_command

Здесь 2>&1 перенаправляет stderr на stdout, а 1>&2 перенаправляет stdout на stderr, таким образом, stdout и stderr команды command будут направлены в pipe, а stdout будет продолжать отображаться на pty.
